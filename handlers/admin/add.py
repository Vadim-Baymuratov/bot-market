...
async def show_products(m, products, category_idx):
    ...
    for idx, title, body, image, price, tag in products:
        ...
        markup.add(InlineKeyboardButton('🗑️ Удалить',
                                        callback_data=product_cb.new(
                                            id=idx, action='delete')))
...
async def process_settings(message: Message):
    ...
    for idx, title in db.fetchall(
            'SELECT * FROM categories',
            None):   # if we don't have second argument we have to put none
    ...
    await message.answer('Настройки категорий:', reply_markup=markup)
